...
>
> 定稿日期：2026-08-28；V2 更新：2026-08-30

## 0. 下一阶段目标（V2）

下一阶段不再以“增加更多绘图原语”为主，而是把现有原语收敛成一条可重复、
可诊断、可交付的原理图生成流水线：
...
- 新建工程自动具备 `.kicad_pro`、`sym-lib-table`、`fp-lib-table`，CLI 与 GUI
  对 `${KIPRJMOD}` 的解析结果一致。
- Windows/Linux 都只使用显式指定或当前仓库构建的 KiCad，不依赖系统安装路径。
- 标签、符号、导线之间保留可配置净距；“不重叠”升级为“至少满足净距”。
- 生成文件保存后关闭并重开，不出现“automatically fixed”提示，第二次保存无结构变化。
- 每次改动至少通过：结构校验、Golden 连通性、ERC 门禁、几何门禁、SVG 视觉回归。

### P0 — 工程上下文与可重开性（先完成）

#### 0.1 工程脚手架

- [x] 新增 `ensure_kicad_project(sch_file)`，在缺失时创建同名 `.kicad_pro`，并校验
  `sym-lib-table` / `fp-lib-table` 中引用的路径存在。
- [x] `.kicad_pro` 使用 KiCad 自身默认项目结构或仓库模板生成，不手拼不完整 JSON。
- [x] `draw_circuit` 在清图前执行工程预检；缺少库、封装、数据手册时提前返回结构化错误，
  不到 ERC 阶段才暴露 45 条重复告警。
- [x] 层级预检递归检查子页存在、顶层 UUID 唯一、实例路径匹配和项目页码唯一，
  在启动 Eeschema 前拦截会触发自动修复的结构问题。
- [x] 临时测试工程也必须是完整 KiCad 工程，禁止只有孤立 `.kicad_sch`。

**验收**：在全新临时目录生成矩阵，仓库 `kicad-cli sch erc` 不出现
`unresolved_variable`、`lib_symbol_issues`、`footprint_link_issues`。

#### 0.2 跨平台资源发现

- [x] 抽出统一 `KiCadRuntime`：解析 `KICAD_CLI`、`KICAD_STOCK_DATA_HOME` 和用户配置目录；
  `render.py`、`symbols.py`、`symbol_browser.py`、`symbol_lib.py` 共用。
- [x] 查找顺序固定为：显式环境变量 -> 当前仓库 install tree；默认禁止回退系统 PATH，
  仅显式 `KICAD_ALLOW_PATH=1` 时允许，并记录最终命中的可执行文件和资源根目录。
- [x] Windows 使用 `%APPDATA%/kicad/<major>`，Linux 使用
  `~/.config/kicad/<major>`，版本从运行时或配置读取，不硬编码 `10.0`。
- [x] 删除 `/tmp/squashfs-root`、`/media/...` 和 Unix-only `PATH` 覆盖。

**验收**：Windows/Linux 单元测试覆盖路径解析；Windows 端到端测试确认进程路径位于
`build/install/msvc-local-release/bin`。

#### 0.3 保存/重载门禁

- [x] 新增原生 `GetSchematicState` 和 `CloseDocument`，查询加载修复/未保存状态并安全关闭
  已保存文档；状态包含宿主 PID，用于 Windows 固定 API socket 场景下精确清理残留进程。
- [x] `kicad_sch_reload_gate` 自动执行保存 -> 关闭 -> 仓库 Eeschema 重开，默认连续两轮；
  每轮检查加载修复、未保存状态和层级工程语义快照。
- [ ] 对 API 创建的 Symbol/Line/Junction/各类 Label/BusEntry 建立单对象和组合测试。
- [x] 重载后再次保存并做语义比较：UUID、实例路径、页码、对象计数和连接图必须稳定；
  忽略纯排序或格式化差异。

**验收**：连续生成、保存、关闭、重开两轮，无修复弹窗且语义 diff 为空。

### P1 — 几何约束与标签质量

#### 1.1 统一几何模型

- [x] 建立 `GeometryModel`，统一计算 Symbol body/pin、Label shape/text、Wire、Junction 的包围盒。
- [x] 标签包围盒必须考虑 `spin`、shape、字高、文本方向和 KiCad text offset；替换
  `overlaps.py::_label_box_mm` 当前只按单一水平方向估算的实现。
- [x] 区分 overlap 与 clearance；默认净距：标签-标签 `1.27mm`、标签-符号 `1.27mm`、
  导线-非所属符号 `1.27mm`，允许在 CircuitSpec 中覆盖。
- [x] 导出的 SVG 反向测量 viewBox、墨迹边界、绘图元素、文字元素和最小文字间距，
  作为 protobuf 几何估算的校准数据。

**验收**：矩阵中任意相邻 `R*` / `C*` 标签真实 SVG 净距 >= `1.27mm`；旋转标签
0/90/180/270 度测试均不漏报。

#### 1.2 标签放置求解

- [x] 标签候选位置由固定 stub 升级为离散候选集：四方向、`1.27mm` 步进、短/中/长 stub。
- [x] 代价函数包含：重叠、净距不足、越界、穿线、离引脚距离、同网标签对齐；硬约束失败时
  不创建对象并返回可定位冲突。
- [x] 同符号同网引脚先归并，再选择离相邻符号最远的主引脚，避免重复标签。
- [x] 将标签净距纳入布局尺寸估算，使布局主动留空间，而不是绘制后依赖 fix-overlaps 搬移。

**验收**：矩阵间距缩小测试仍能自动扩列或调整标签，无接触、无越界、Golden 不变。

### P1 — 布局与布线质量

#### 1.3 约束布局器

- [x] 将布局拆成“拓扑分层 -> 列内排序 -> 尺寸约束 -> 页面装箱”四步，并输出每步诊断。
- [x] 元件间距由符号本体、引脚扇出、标签占用和预留布线通道共同决定。
- [x] 支持功能组、固定位置、对齐、同行/同列、电源区、禁止区和标题栏避让。
- [x] 大图超出单页时给出分页建议，禁止静默压缩到不可读间距。

**指标**：符号重叠 0、越界 0、可配置净距违规 0；常用电源/MCU/矩阵样例布局稳定。

#### 1.4 拓扑布线器

- [x] 建立占用网格和 net ownership，线段、引脚、Junction 都带所属网络。
- [x] 路由前检测零长度、重复、共线重叠、异网 T 接和端点落入异网线段中部。
- [x] 先路由电源轨和高扇出网，再路由普通网；失败网络使用 A* 正交寻路，而不是无限增加 lane。
- [x] Junction 只在同网三路及以上连接处创建；交叉不同网禁止创建 Junction。
- [x] 路由结果先在内存连接图中验证，再批量提交 API；失败时整批回滚。

**指标**：异网短路 0、off-grid 0、非必要交叉数量可量化且不高于基线。

### P2 — 验证、可观测性与回归体系

#### 2.1 分层门禁

每次 `draw_circuit` 返回结构化 `DrawReport`，依次执行：

1. **Schema gate**：CircuitSpec 字段、位号、库、引脚和网络引用有效。
2. **Create gate**：请求对象数等于 API 返回对象数，失败对象带 ref/net/阶段。
3. **Topology gate**：读回连接图与输入网络一致。
4. **ERC gate**：blocking 必须为 0；豁免必须显式列出规则和原因。
5. **Geometry gate**：重叠、净距、越界、穿线均为 0。
6. **Reload gate**：关闭重开后未被 KiCad 自动修复。
7. **Visual gate**：SVG 元素计数、页面范围、最小净距和基线快照通过。

任何门禁失败都停止后续“自动修复”，保留阶段产物和机器可读报告，避免错误被后续步骤覆盖。

#### 2.2 测试矩阵

| 层级 | 测试 | 必须覆盖 |
|---|---|---|
| 单元 | 无 KiCad 进程 | 路径发现、CircuitSpec、旋转 bbox、净距、网格路由 |
| API 集成 | 仓库 Eeschema | 每种对象 create/get/delete/save/reload |
| Golden | 连接图 | RC、分压、电源、MCU、矩阵、Bus、多页 |
| CLI | 仓库 kicad-cli | ERC、SVG 导出、项目变量和本地库解析 |
| 视觉 | SVG/截图 | 标签净距、交叉、越界、标题栏遮挡 |
| 失败注入 | 非法输入 | 缺库、缺项目、off-grid、重复 UUID、异网短路 |

### 实施顺序

| 迭代 | 内容 | 预计 | 退出条件 |
|---|---|---:|---|
| V2.1 | 工程脚手架 + `KiCadRuntime` | 2-3 天 | 临时工程 CLI ERC 无资源类告警 |
| V2.2 | Reload gate + API 对象组合测试 | 2-3 天 | 连续两次重开无自动修复 |
| V2.3 | Spin-aware 几何与 clearance gate | 3-4 天 | 矩阵最小标签净距 >= 1.27mm |
| V2.4 | 约束布局与标签候选求解 | 4-6 天 | 典型样例无需事后搬移 |
| V2.5 | 占用网格 + A* 路由兜底 | 5-8 天 | 高扇出样例无短路/off-grid |
| V2.6 | 结构化报告 + CI 回归 | 2-3 天 | 一条命令完成全门禁并输出产物 |

**V2.3-V2.6 验收记录（2026-08-30）**：

- V2.3：四向标签、符号/导线/Junction bbox、`1.27mm` clearance 与 SVG 实测已接入。
- V2.4：功能组/固定/同行同列/禁止区约束和四向标签候选求解已接入，失败时不创建对象。
- V2.5：电源及高扇出优先、net-owned 占用网格、两引脚/多引脚 A* 与提交前拓扑检查已接入。
- V2.6：`DrawReport`、七层质量流水线、Golden、SVG 产物及 `kicad-mcp-quality` 命令已接入。
- keyboard-89 实测：3 页、232 个连接网络、ERC blocking 0、1.27mm 几何门禁通过、
  连续两轮重载无修复、40647 个 SVG 绘图元素均位于页面内、重复 Golden 比较通过。

### 本轮优先落点

1. 先实现 `ensure_kicad_project()`，修复临时工程缺少 `.kicad_pro` 导致的资源类 ERC 告警。
2. 合并四处 KiCad 路径发现逻辑，保证只使用当前仓库构建。
3. 修正 spin-aware 标签包围盒并加入 `1.27mm` clearance 检查。
4. 增加新建矩阵 -> 保存 -> 重开 -> Golden/ERC/SVG 的 Windows 端到端测试。

这四项完成后，才进入布局器和路由器的大改，避免在不稳定工程环境上调算法。
